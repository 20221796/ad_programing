...
char getTetrisKey(TetrisState state, bool fromUser, bool toFile) {
  ...
  if (toFile == true) {
    ...
    // outfStream << key;
    recorded_keys.push_back(key); // 2-1
    key_cnt++;

    if (key_cnt == 9) { 
      // for(int i=0; i<key_cnt; i++)
      // outfStream << recorded_keys;
      // string *sp = 
      outfStream.write(recorded_keys.c_str(), 10); //2-2
      recorded_keys = "";
      key_cnt = 0;
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  if (outfStream.is_open() == true) 
    // outfStream << recorded_keys;
    outfStream.write(recorded_keys.c_str(), recorded_keys.length()); //2-3
    recorded_keys = "";
    key_cnt = 0;
    outfStream.close();
